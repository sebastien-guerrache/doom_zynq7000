...
extern boolean_doom		message_dontfuckwithme;

extern boolean_doom		chat_on;		// in heads-up code
...
boolean_doom			messageNeedsInput;
...
boolean_doom			inhelpscreens;
boolean_doom			menuactive;

#define SKULLXOFF		-32
#define LINEHEIGHT		16

extern boolean_doom		sendpause;
char			savegamestrings[10][SAVESTRINGSIZE];

char	endstring[160];

//static boolean_doom opldev;
...
void M_StartMessage(char *string,void *routine,boolean_doom input);
...
void M_LoadGame (int choice)
{
    if (netgame)
    {
	M_StartMessage(DEH_String(LOADNET),NULL,doom_false);
	return;
    }
...
void M_SaveGame (int choice)
{
    if (!usergame)
    {
	M_StartMessage(DEH_String(SAVEDEAD),NULL,doom_false);
	return;
    }
...
void M_QuickLoad(void)
{
    if (netgame)
    {
	M_StartMessage(DEH_String(QLOADNET),NULL,doom_false);
	return;
    }
	
    if (quickSaveSlot < 0)
    {
	M_StartMessage(DEH_String(QSAVESPOT),NULL,doom_false);
	return;
    }
...
void M_NewGame(int choice)
{
    ...
    {
	M_StartMessage(DEH_String(NEWGAME),NULL,doom_false);
	return;
    }
...
void M_ChooseSkill(int choice)
{
    if (choice == nightmare)
    {
	M_StartMessage(DEH_String(NIGHTMARE),M_VerifyNightmare,doom_true);
	return;
    }
...
void M_Episode(int choice)
{
    if ( (gamemode == shareware)
	 && choice)
    {
	M_StartMessage(DEH_String(SWSTRING),NULL,doom_false);
...
void M_EndGame(int choice)
{
    ...
    if (netgame)
    {
	M_StartMessage(DEH_String(NETEND),NULL,doom_false);
	return;
    }
	
    M_StartMessage(DEH_String(ENDGAME),M_EndGameResponse,doom_true);
}
...
static boolean_doom IsNullKey(int key)
{
...
// M_Responder
//
boolean_doom M_Responder (event_t* ev)
{
    int             ch;
    int             key;
    int             i;
