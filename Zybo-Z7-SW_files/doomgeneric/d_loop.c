...
typedef struct
{
    ...
    boolean_doom ingame[NET_MAXPLAYERS];
} ticcmd_set_t;
...
// When set to doom_true, a single tic is run each time TryRunTics() is called.
// This is used for -timedemo mode.

boolean_doom singletics = doom_false;
...
static boolean_doom  new_sync = doom_true;
...
static boolean_doom local_playeringame[NET_MAXPLAYERS];
...
static boolean_doom BuildNewTic(void)
{
    int	gameticdiv;
    ticcmd_t cmd;
    ...
    ticdata[maketic % BACKUPTICS].ingame[localplayer] = doom_true;
...
void D_ReceiveTic(ticcmd_t *ticcmds, boolean_doom *players_mask)
{
    int i;
...
boolean_doom D_InitNetGame(net_connect_data_t *connect_data)
{
    boolean_doom result = doom_false;
#ifdef FEATURE_MULTIPLAYER
    ...
    I_AtExit(D_QuitNetGame, doom_true);
...
// Returns doom_true if there are players in the game:

static boolean_doom PlayersInGame(void)
{
    boolean_doom result = doom_false;
    unsigned int i;
