...
boolean_doom		devparm;	// started game with -devparm
boolean_doom         nomonsters;	// checkparm of -nomonsters
boolean_doom         respawnparm;	// checkparm of -respawn
boolean_doom         fastparm;	// checkparm of -fast

//extern int soundVolume;
//extern  int	sfxVolume;
//extern  int	musicVolume;

extern  boolean_doom	inhelpscreens;

skill_t		startskill;
int             startepisode;
int		startmap;
boolean_doom		autostart;
int             startloadgame;

boolean_doom		advancedemo;

// Store demo, do not accept any inputs
boolean_doom         storedemo;

// "BFG Edition" version of doom2.wad does not include TITLEPIC.
boolean_doom         bfgedition;

// If doom_true, the main game loop has started.
boolean_doom         main_loop_started = doom_false;
...
extern  boolean_doom setsizeneeded;
extern  int             showMessages;
void R_ExecuteSetViewSize (void);

void D_Display (void)
{
    static  boolean_doom		viewactivestate = doom_false;
    static  boolean_doom		menuactivestate = doom_false;
    static  boolean_doom		inhelpscreensstate = doom_false;
    static  boolean_doom		fullscreen = doom_false;
...
boolean_doom D_GrabMouseCallback(void)
{
...
void D_DoomLoop (void)
{
	xil_printf("Doom_tick test entering, d_main.c\r\n");
    if (bfgedition &&
    ...
    main_loop_started = doom_true;
    xil_printf("Doom_tick test main loop, d_main.c\r\n");
    TryRunTics();
    xil_printf("Doom_tick before tryruntics, d_main.c\r\n");
    I_SetWindowTitle(gamedescription);
    xil_printf("Doom SetWindowTitle Done, d_main.c\r\n");
    I_GraphicsCheckCommandLine();
    xil_printf("Doom GraphicsCheckCommandLine Done, d_main.c\r\n");
    I_SetGrabMouseCallback(D_GrabMouseCallback);
    xil_printf("Doom SetGrabMouseCallback Done, d_main.c\r\n");
    I_InitGraphics();
    xil_printf("Doom InitGraphics Done, d_main.c\r\n");
    I_EnableLoadingDisk();
    ...
    xil_printf("Doom_tick test, d_main.c\r\n");
    doomgeneric_Tick();
}
...
{
    boolean_doom is_freedoom = W_CheckNumForName("FREEDOOM") >= 0,
...
static boolean_doom D_AddFile(char *filename)
{
    wad_file_t *handle;
...
static void LoadIwadDeh(void)
{
    ...
    if (W_CheckNumForName("FREEDOOM") >= 0)
    {
        ...
        // is an old IWAD.
        DEH_LoadLumpByName("DEHACKED", doom_false, doom_true);
    }

    // If this is the HACX IWAD, we need to load the DEHACKED lump.
    if (gameversion == exe_hacx)
    {
        if (!DEH_LoadLumpByName("DEHACKED", doom_true, doom_false))
        {
...
void D_DoomMain (void)
{
    ...
    I_AtExit(M_SaveDefaults, doom_false);
...
//    W_PrintDirectory();
    ...
    I_AtExit((atexit_func_t) G_CheckDemoStatus, doom_true);
    ...
    if (M_ParmExists("-dehlump"))
    {
        ...
        for (i = numiwadlumps; i < numlumps; ++i)
        {
            ...
            {
                DEH_LoadLump(i, doom_false, doom_false);
                loaded++;
            }
        }
    ...
    xil_printf("Entering Doom loop, d_main.c\r\n");
    D_DoomLoop ();
}
