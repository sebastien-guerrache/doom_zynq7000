...
boolean_doom         paused; 
boolean_doom         sendpause;             	// send a pause event next tic 
boolean_doom         sendsave;             	// send a save event next tic 
boolean_doom         usergame;               // ok to save / end game 
 
boolean_doom         timingdemo;             // if doom_true, exit with report on completion 
boolean_doom         nodrawers;              // for comparative timing purposes 
int             starttime;          	// for comparative timing purposes  	 
 
boolean_doom         viewactive; 
 
int             deathmatch;           	// only if started as net death 
boolean_doom         netgame;                // only doom_true if packets are broadcast 
boolean_doom         playeringame[MAXPLAYERS]; 
player_t        players[MAXPLAYERS]; 

boolean_doom         turbodetected[MAXPLAYERS];
...
char           *demoname;
boolean_doom         demorecording; 
boolean_doom         longtics;               // cph's doom 1.91 longtics hack
boolean_doom         lowres_turn;            // low resolution turning for longtics
boolean_doom         demoplayback; 
boolean_doom		netdemo; 
byte*		demobuffer;
byte*		demo_p;
byte*		demoend; 
boolean_doom         singledemo;            	// quit after playing a demo from cmdline 
 
boolean_doom         precache = doom_true;        // if doom_true, load all graphics at start 

boolean_doom         testcontrols = doom_false;    // Invoked by setup to test controls
...
static boolean_doom  gamekeydown[NUMKEYS]; 
static int      turnheld;		// for accelerative turning 
 
static boolean_doom  mousearray[MAX_MOUSE_BUTTONS + 1];
static boolean_doom *mousebuttons = &mousearray[1];  // allow [-1]
...
static boolean_doom  dclickstate;
static int      dclicks; 
static int      dclicktime2;
static boolean_doom  dclickstate2;
static int      dclicks2;
...
static boolean_doom  joyarray[MAX_JOY_BUTTONS + 1]; 
static boolean_doom *joybuttons = &joyarray[1];		// allow [-1] 
...
static boolean_doom WeaponSelectable(weapontype_t weapon)
{
...
// 
boolean_doom G_Responder (event_t* ev) 
{ 
...
boolean_doom G_CheckDemoStatus (void) 
{ 
    int             endtime; 
