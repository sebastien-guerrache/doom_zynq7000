...
struct _net_module_s
{
    ...
    boolean_doom (*InitClient)(void);

    // Initialize this module for use as a server

    boolean_doom (*InitServer)(void);
    ...
    //
    // Returns doom_true if packet received

    boolean_doom (*RecvPacket)(net_addr_t **addr, net_packet_t **packet);
...
typedef struct 
{
    signed int latency;
    unsigned int seq;
    boolean_doom playeringame[NET_MAXPLAYERS];
