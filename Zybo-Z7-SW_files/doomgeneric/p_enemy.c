...
// P_CheckMeleeRange
//
boolean_doom P_CheckMeleeRange (mobj_t*	actor)
{
    mobj_t*	pl;
    fixed_t	dist;
...
// P_CheckMissileRange
//
boolean_doom P_CheckMissileRange (mobj_t* actor)
{
    fixed_t	dist;
...
boolean_doom P_Move (mobj_t*	actor)
{
    fixed_t	tryx;
    fixed_t	tryy;
...
//
boolean_doom P_TryWalk (mobj_t* actor)
{	
    if (!P_Move (actor))
    {
	return doom_false;
...
boolean_doom PIT_VileCheck (mobj_t*	thing)
{
    int		maxdist;
    boolean_doom	check;
...
static boolean_doom CheckBossEnd(mobjtype_t motype)
{
